...
def go_to_furthest(num:str, direction:str, final_state:str):
    ...
    find_new_zero = get_to_a_zero(num+"b", direction, final_state, 1)
...
def get_to_a_zero(num:str, direction:str, final_state:str, depth:int):
    final_lines = []
    keep_moving = [[num, "1", "1", direction, num], 
                   [num, "2", "2", direction, num], 
                   [num, "3", "3", direction, num], ]
    if (depth == 0):
        gets_to_end = go_to_furthest(num, direction, final_state)
    ...
    for line in lines:
        ...
        if input_letter == ZERO or input_letter == STAR:
            ...
            if input_letter == ZERO:            
                ...
                new_rules = get_to_a_zero(str(counter), direction, output_state, 0)
                counter += 1
